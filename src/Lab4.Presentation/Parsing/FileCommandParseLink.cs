...
public class FileCommandParseLink : CommandParseLinkBase
{
    public override ParsedCommand? Parse(string line)
    {
        string[] parts = line.Split(' ');
        if (parts.Length == 0) return null;
        if (parts[0] != "file") return CallNext(line);
        if (parts.Length < 2) throw new ArgumentException("file requires subcommand");
        ...
        return new ParsedCommand("file", sub, args, flags);
    }
}